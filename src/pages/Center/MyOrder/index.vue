            ...
            <tr v-for="(goods, index) in order.orderDetailList" :key="goods.id">
              <td width="60%">
                <div class="typographic">
                  <img :src="goods.imgUrl" style="width: 82px; height: 82px" />
...
export default {
  ...
  methods: {
    async getMyOrderList() {
      let result = await this.$API.reqGetMyOrderList(this.page, this.limit);
      if (result.code == 200) {
    ...
    changePageNo(flag) {
      ...
      if (flag === "pre") {
        this.page = pageNo > 1 ? pageNo - 1 : pageNo;
      } else if (flag === "next") {
        this.page = pageNo < this.myOrderInfo.pages ? pageNo + 1 : pageNo;
      } else {
        this.page = flag;
      }
      this.getMyOrderList();
